...
| `battle.mjs` | the turn-based fight: move sets, the type chart beating the stat gap, the catch roll inheriting the Restraint maths, priority, swapping, packs fought and recorded one member at a time, an apex fight that is a fight rather than an endurance test, every apex phase actually happening and gating its own capture, Nyxhollow blanking its health bar until you bring a Lumen, Study going only to whoever was on the field, and that a Warden with no monsters can still win their first fight |
